...
double Calculator::Word(string str, int* i) {
	...
	if (word.compare("sqrt") == 0) {
		Single* s = new Single(functions->Function(SQRT));
		return(s->Execute(calc->Result()));
	}
	if (word.compare("exp") == 0) {
		Single* s = new Single(functions->Function(EXP));
		return(s->Execute(calc->Result()));
	}
	if (word.compare("int") == 0) {
		Single* s = new Single(functions->Function(INt));
		return(s->Execute(calc->Result()));
	}
}
